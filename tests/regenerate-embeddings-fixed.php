...
echo "=== Regenerate Embeddings with Date Emphasis (Fixed) ===" . PHP_EOL;

require_once 'vendor/autoload.php';
$app = require_once 'bootstrap/app.php';
$app->make('Illuminate\Contracts\Console\Kernel')->bootstrap();

echo "This will regenerate embeddings with enhanced date information using correct database schema." . PHP_EOL;
echo "" . PHP_EOL;
...
// Enhanced embedding generation function using correct schema
function generateEnhancedEmbeddingContent($entity, $entityType) {
    ...
    switch ($entityType) {
        case 'bill':
            ...
            // Add sponsor information if available (from joined data)
...
echo "✅ Use correct database schema (bills + bill_sponsors tables)" . PHP_EOL;
...
$billChunks = DB::table('bills')
    ->leftJoin('bill_sponsors', function($join) {
        $join->on('bills.id', '=', 'bill_sponsors.bill_id')
             ->where('bill_sponsors.is_primary', '=', true);
    })
    ->select(
        'bills.id', 
        'bills.type', 
    ...
    ->chunk(50, function ($bills) use ($embeddingService, &$totalProcessed, &$totalErrors, $startTime) {
...
echo "✅ Fixed database schema issues" . PHP_EOL;
echo "" . PHP_EOL;
