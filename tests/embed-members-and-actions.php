...
echo "=== Generate Member and Action Embeddings ===" . PHP_EOL;

require_once 'vendor/autoload.php';
$app = require_once 'bootstrap/app.php';
$app->make('Illuminate\Contracts\Console\Kernel')->bootstrap();

echo "This will generate embeddings for members and bill actions only." . PHP_EOL;
echo "This is much faster than processing all bills!" . PHP_EOL;
echo "" . PHP_EOL;
...
function generateMemberEmbeddingContent($member) {
    ...
    if (!empty($member->biography)) {
        $bio = substr($member->biography, 0, 200);
...
$processedMembers = 0;
DB::table('members')
    ->select('id', 'full_name', 'party_abbreviation', 'state', 'chamber', 'district', 'current_member', 'biography')
